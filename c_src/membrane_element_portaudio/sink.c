...
void handle_destroy_state(UnifexEnv *env, SinkState* state) {
  ...
  UnifexPid exec_pid;
  if(!unifex_get_pid_by_name(env, "Elixir.Membrane.Element.PortAudio.SyncExecutor", &exec_pid)
...
static int callback(const void *_input_buffer, void *output_buffer, unsigned long frames_per_buffer, const PaStreamCallbackTimeInfo* _time_info, PaStreamCallbackFlags _flags, void *user_data) {
  ...
  size_t elements_available = membrane_ringbuffer_get_read_available(state->ringbuffer);
  if(elements_available >= frames_per_buffer) {
    size_t elements_read = membrane_ringbuffer_read(state->ringbuffer, output_buffer, frames_per_buffer);
    if(state->demand+elements_read > state->ringbuffer->max_elements/2) {
      if(!send_demand(env, state->demand_handler, UNIFEX_SEND_THREADED, (state->demand+elements_read)*FRAME_SIZE)) {
...
UNIFEX_TERM create(UnifexEnv* env, UnifexPid demand_handler, int endpoint_id, int ringbuffer_size, int pa_buffer_size, char* latency) {
  ...
  send_demand(env, demand_handler, UNIFEX_NO_FLAGS, ringbuffer_size*FRAME_SIZE);
...
UNIFEX_TERM write_data(UnifexEnv* env, UnifexPayload* payload, SinkState* state) {
  size_t elements_written = membrane_ringbuffer_write(state->ringbuffer, payload->data, payload->size / FRAME_SIZE);
  if(elements_written != payload->size / FRAME_SIZE) {
    MEMBRANE_WARN(env, "Write: written only %d out of %lu bytes into ringbuffer", elements_written * FRAME_SIZE, payload->size);
