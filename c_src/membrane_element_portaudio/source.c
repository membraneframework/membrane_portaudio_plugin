...
static int callback(const void *input_buffer, void *_output_buffer, unsigned long frames, const PaStreamCallbackTimeInfo* _time_info, PaStreamCallbackFlags _flags, void *user_data) {
  ...
  UnifexPayload* payload = unifex_payload_alloc(env, UNIFEX_PAYLOAD_BINARY, frames * FRAME_SIZE);
...
UNIFEX_TERM create(UnifexEnv* env, UnifexPid destination, int endpoint_id, int pa_buffer_size, char* latency) {
